...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#goInsert").click(function(){
			$("#bookListAction").attr("action", "/infoBookInsert");
			$("#bookListAction").submit();
		});
		$("#bookSearch").click(function(){
			$("#bookListAction").attr("action", "/infoBookList");
			$("#bookListAction").submit();
		});
	});
</script>
				...
				<table>
					...
					<c:forEach var="ib" items="${infoBookList}" begin="${pageHelper.startRow}" end="${pageHelper.lastRow}" step="1">
						<tr>
							...
							<td><a href="/infoBookDetail?infoBookCode=${ib.infoBookCode}&searchType=${searchType}&searchWord=${searchWord}">${ib.infoBookName}</a></td>
						</tr>
					</c:forEach>
				</table>
				<div>
